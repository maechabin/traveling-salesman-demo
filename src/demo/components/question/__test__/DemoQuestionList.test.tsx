...
describe('<DemoQuestionList />', () => {
  ...
  it('経路リストにsortIdが付与されていない場合、Id順（昇順）に表示されること', () => {
    ...
    expect(wrapper.find('.DemoQuestionListRoutes').text()).toBe(
      '経路A:秋葉原駅経路B:文京区役所経路C:明治大学経路D:浜離宮経路E:国会議事堂経路F:東京タワー経路G:上野動物園経路H:東京駅',
    );
  });
  ...
  it('経路リストにsortIdが付与されていた場合、sortId順（降順）に表示されること', () => {
    ...
    expect(wrapper.find('.DemoQuestionListRoutes').text()).toBe(
      '経路E:国会議事堂経路G:上野動物園経路H:東京駅経路A:秋葉原駅経路B:文京区役所経路F:東京タワー経路D:浜離宮経路C:明治大学',
    );
  });
  ...
  it('sortIdが0以上であった場合、sortIdが優先してソートされること', () => {
    ...
    expect(wrapper.find('.DemoQuestionListRoutes').text()).toBe(
      '経路E:国会議事堂経路G:上野動物園経路H:東京駅経路A:秋葉原駅経路B:文京区役所経路C:明治大学経路D:浜離宮経路F:東京タワー',
    );
  });
  ...
  it('answerWaypointOrderの要素数が0であれば、class名にDemoQuestionListMistakeを持ったリストはないこと', () => {
    ...
    const classNames = wrapper
      .find('.DemoQuestionListRoutes')
      .props()
      .children.map(li => li.props.className);
  ...
  it('正解の経路リストと一致しなかった場合は、class名にDemoQuestionListMistakeが付与されること', () => {
    ...
    const mistakedList = wrapper
      .find('.DemoQuestionListRoutes')
      .props()
      .children.filter((li, i) => answerWaypointOrder[i] !== i);
    mistakedList.map(li =>
